...
public class PrepaidToPostpaid extends BasePage {
	public PrepaidToPostpaid(TestContext context) {
		super(context);
	}
		//Migration Page
				...
				private SelenideElement lst_bill_medium = $(By.xpath("//div[@name='billDispatchDetails']//li[normalize-space()='Email']"));
				...
				public void prepaid_postpaid(Customer customer,String planname) throws InterruptedException {
				...
				public void capture_vas_page() {
					...
					public void capture_customer_page(Customer customer) {
						...
						ExtentCucumberAdapter.addTestStepLog("Select Language as " + customer.profileDetails.notificationDetails.language);
			       ...
			       public void capture_billing_page(Customer customer) {
			    	if(txt_billing_name.exists()) {
			    		...
			    		ExtentCucumberAdapter.addTestStepLog("Enter Billing Name as " + customer.profileDetails.basicDetails.firstName);
			    	} 
			    	...
			    	txt_fname.shouldHave(value(customer.profileDetails.basicDetails.firstName));
			   		ExtentCucumberAdapter.addTestStepLog("Validated Firstname as " + customer.profileDetails.basicDetails.firstName);
					...
					ExtentCucumberAdapter.addTestStepLog("Select Releationship Manager  as " + customer.accountDetails.others.relationshipManager);
					//ACCOUNT TYPE
			    	...
			    	ExtentCucumberAdapter.addTestStepLog("Selected Bill Cycle as " + customer.accountDetails.operatorAccountManager.billCycle);
					...
					ExtentCucumberAdapter.addTestStepLog("Enter Bill Dispatch Email as " + customer.profileDetails.notificationDetails.email);
					lst_Region.selectOption((customer.accountDetails.operatorAccountManager.billingRegion));
					ExtentCucumberAdapter.addTestStepLog("Selected Bill Region as " + customer.accountDetails.operatorAccountManager.billingRegion);
					lst_bill_currency.selectOption((customer.accountDetails.operatorAccountManager.billCurrency));
					ExtentCucumberAdapter.addTestStepLog("Selected Bill Currency as " + customer.accountDetails.operatorAccountManager.billCurrency);
